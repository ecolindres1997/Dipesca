    ...
    <div class="row">
      <div class=" col-sm-12 col-md-12">
        <div class="form-group">
          <label for="nombre1">Nombre de la comunidad</label>
          <input type="text" name="NombreComunidad" class="form-control" required
                  value="<?php echo $NombreComunidad; ?>"placeholder="Ingrese el nombre">
        </div>
      </div>
    </div>
    <div class="row">
      <div class=" col-sm-12 col-md-12">
       <div class="form-group">
                  <label for="genero">Nombre del Area Geografica</label>
                   <select  class="form-control" name="IdAgeo">
                    <option value="0" ><?php echo "$IdAgeo"; ?></option>
                    <?php
                      $query = $conexion -> query ("SELECT * from  litoral");
                     
                      while ($IdAgeo = mysqli_fetch_array($query)) {
                        echo '<option value="'.$IdAgeo[id].'">'.$IdAgeo[nombre_litoral].'</option>';
                      } 
                    ?>
                  </select>
                </div>
